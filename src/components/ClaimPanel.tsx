...
export function ClaimPanel({ address }: ClaimPanelProps) {
  ...
  const { will, beneficiaries } = useWill(lookedUp);
  const hasClaimed = useHasClaimed(lookedUp, address);
  const isBeneficiary = beneficiaries.includes(address);

  // Can claim if: will is active, triggered, is beneficiary, and hasn't claimed yet
  const canClaim = lookedUp && will?.isActive && will?.isTriggered && isBeneficiary && !hasClaimed;
  ...
  }, [will, lookedUp, isBeneficiary, hasClaimed, canClaim]);
      ...
      {lookedUp && will && (
        <div className="border border-white/10 bg-white/5 p-5 grid grid-cols-2 gap-4">
          {[
            ...
            { label: "Will Can be triggered now!", value: will.isTriggered ? "Yes (Overdue)" : "No", ok: will.isTriggered },
